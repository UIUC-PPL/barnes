...
void TreePiece::startSph(){
  CkPrintf("Starting Sph calucations.\n");
  sphTraversalState.reset(myNumParticles, globalParams.nSmooth);
  localSphTraversalWorker.reset(this, &sphTraversalState, *myBuckets);
  doLocalSph(); 
}

void TreePiece::doLocalSph(){
  trav.bottomUpTraversal(root, &localSphTraversalWorker, &sphTraversalState);
  //thisProxy[thisIndex].doRemoteSph();
}

void TreePiece::doRemoteSph(){
  //trav.bottomUpTraversal(root, &remoteSphTraversalWorker, &sphTraversalState);
  //sphTraversalDone();
}
void TreePiece::sphTraversalDone(){
    sphDone();
}
...
void TreePiece::traversalDone(){ 
  ...
  if(numTraversalsDone == totalNumTraversals){
     thisProxy[thisIndex].startSph();
  }
}

void TreePiece::sphDone(){
#ifdef STATISTICS
